...
const StatsPanel: React.FC<StatsPanelProps> = ({ 
  ...
  return (
    <div className="w-full lg:w-96 flex flex-col gap-6 animate-in fade-in slide-in-from-right duration-700">
      ...
      <div className="bg-surface/60 backdrop-blur-xl border border-white/5 rounded-3xl p-6 relative overflow-hidden group">
        <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
           <Zap size={80} />
        </div>
        
        <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-4">Current Task</h3>
        
        <div className="mb-6">
          <h2 className="text-xl font-bold text-white mb-1">{focusTask}</h2>
        ...
        <button 
          ...
          className="w-full py-3 bg-white/5 hover:bg-white/10 text-slate-300 font-medium rounded-xl transition-all flex items-center justify-center gap-2 border border-white/5"
        >
      ...
      {/* Daily Stats */}
      <div className="bg-surface/40 backdrop-blur-lg border border-white/5 rounded-3xl p-6 flex-1">
         <div className="flex items-center justify-between mb-6">
            <h3 className="text-sm font-bold text-white">Today's Progress</h3>
            ...
            {editingGoal ? (
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="w-12 px-2 py-1 bg-white/10 border border-white/20 rounded text-center text-white text-xs"
                />
                <button onClick={handleSaveGoal} className="p-1 hover:bg-white/10 rounded">
         ...
         {/* Progress Bar */}
         <div className="w-full h-3 bg-slate-800 rounded-full mb-2 overflow-hidden">
            <div 
         ...
         <div className="grid grid-cols-2 gap-4 mb-6">
            <div className="bg-white/5 p-4 rounded-2xl border border-white/5 hover:border-white/10 transition-colors">
               <p className="text-xs text-slate-500 mb-1">Total Focus</p>
               <p className="text-2xl font-mono text-white">{stats.totalMinutes}<span className="text-sm text-slate-500 ml-1">m</span></p>
            </div>
            <div className="bg-white/5 p-4 rounded-2xl border border-white/5 hover:border-white/10 transition-colors">
               <p className="text-xs text-slate-500 mb-1">Avg Session</p>
         ...
         <div className="h-24 w-full mb-4">
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={(() => {
                ...
                <Tooltip 
                  contentStyle={{ backgroundColor: '#0B101E', borderColor: '#334155', borderRadius: '8px' }}
                  itemStyle={{ color: '#fff' }}
         ...
         {showHistory && (
           <div className="mt-4 max-h-48 overflow-y-auto space-y-2">
             {todaySessions.length === 0 ? (
               <p className="text-sm text-slate-500 text-center py-4">No sessions completed today</p>
             ) : (
               todaySessions.map((session) => (
                 ...
                 >
                   <div>
                     ...
                     <p className="text-xs text-slate-500">{session.type} • {session.duration}min</p>
                   </div>
                   <span className="text-xs text-slate-400">{formatSessionTime(session.completedAt)}</span>
                 </div>
               ))
             )}
             {todaySessions.length > 0 && (
